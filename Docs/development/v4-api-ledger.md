...
Compared with `55ff7a1`, the default has **57.4% fewer** types, UI has **43.5%
fewer**, and the complete package has **14.9% fewer**. Compared with the coexistence
checkpoint, cleanup removes 62 top-level types across products. Default-product
...
100/52/223, with the original targets retained separately in
...
- The default retains reusable services/utilities, typed settings/pickers,
  barcode/photo presentation, and UIKit interoperability. Platform availability
  is handled at direct call sites; the former compatibility wrapper module has
  been removed. These are distinct capabilities, not obsolete bootstrap or
...
2. **Currency (`Currency/`, 84 declarations, 2 files, 58 currencies + exchange-rate
   ...
   documented decision.
3. **Most of `Services/` and `UI/`'s "secondary" components** (barcode scanner, empty
