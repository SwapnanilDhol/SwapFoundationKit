...
Implement `Scripts/v4-acceptance.py` and source-only negative fixtures/tests after
the source cleanup. The integrator owns the catalog data and reviewed final budgets.
...
The inventory comparison and static acceptance checks run in separate CI jobs.

## Inputs and invariants

- Read `Docs/development/api-baseline.txt`, produced by the unchanged symbol-graph
  methodology. Validate the baseline in CI with `api-baseline.sh --check` before
...
- Make dependencies explicit: if PyYAML is used, install it in an isolated Python
  environment in CI rather than assuming a runner-global install.
- Add checks to CI alongside existing dependency/API gates. Pin Xcode consistently
  to 26.6 for reproducible inventory; select an available simulator dynamically for
  the package suite rather than hardcoding a simulator UUID or name.
